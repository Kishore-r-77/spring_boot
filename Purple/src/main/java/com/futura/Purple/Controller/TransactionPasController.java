...
import com.futura.Purple.Service.TransactionPasService;

@RestController
@RequestMapping("/transaction")
public class TransactionPasController {

	@Autowired
	private TransactionPasService transactionService;
	...
	public ResponseEntity<?> viewAll(@PathVariable Long userId) {
		String method = "get-all-transaction";
	...
	public ResponseEntity<List<?>> getAllPolicyNo() {
		return ResponseEntity.ok(transactionService.getAllPolicyNo());
	}

	@GetMapping("/getByPolicyNo/{policyNo}")
	public TransactionPas findByPolicyNo(@PathVariable Long policyNo) {
		return transactionService.findByPolicyNo(policyNo);
	}

	@GetMapping("/getProcessedByPolicyNo/{policyNo}")
	public TransactionPas findProcessedByPolicyNo(@PathVariable Long policyNo) {
		return transactionService.findProcessedByPolicy(policyNo);
	}

	@GetMapping("/{id}/{userId}")
	public ResponseEntity<?> view(@PathVariable Long id, @PathVariable Long userId) {

		String method = "get-transaction";
		long userGroupId = userRepo.getById(userId).getUserGroupId();
	...
	public ResponseEntity<?> add(@RequestBody TransactionPas entity, @PathVariable Long userId) {

		String method = "add-transaction";
		long userGroupId = userRepo.getById(userId).getUserGroupId();

		Long val =transactionPasRepository.existsByFlcTransNo(entity.getFlcTransNo());
		if (val>0) {
			return ResponseEntity.badRequest().body(new MessageResponse("Client Number already exists"));
		} else {

			if (!permissionRepo.isMethodPresent(userGroupId, serviceId, method).isEmpty()) {
				return ResponseEntity.ok(transactionService.add(entity));
			} else if (!permissionRepo.isMethodPresentUser(userId, serviceId, method).isEmpty()) {
				return ResponseEntity.ok(transactionService.add(entity));
			} else {
				return ResponseEntity.badRequest().body(errorService.getErrorById("ER007"));
	...
	public ResponseEntity<?> update(@RequestBody TransactionPas entity, @PathVariable Long id,
			@PathVariable Long userId) {

		String method = "update-transaction";
		long userGroupId = userRepo.getById(userId).getUserGroupId();
	...
	public List<TransactionPas> search(@PathVariable String val) {
		return transactionService.search(val);
	}

	@DeleteMapping("/hardDelete/{id}/{userId}")
	public ResponseEntity<?> hardDelete(@PathVariable Long id, @PathVariable Long userId) {

		String method = "hard-delete-transaction";
		long userGroupId = userRepo.getById(userId).getUserGroupId();

		if (!permissionRepo.isMethodPresent(userGroupId, serviceId, method).isEmpty()) {
			return ResponseEntity.ok(transactionService.hardDelete(id));
		} else if (!permissionRepo.isMethodPresentUser(userId, serviceId, method).isEmpty()) {
			return ResponseEntity.ok(transactionService.hardDelete(id));
		} else {
			return ResponseEntity.badRequest().body(errorService.getErrorById("ER007"));
	...
	public ResponseEntity<?> softDelete(@PathVariable Long id, @PathVariable Long userId) {
		...
		String method = "soft-delete-transaction";
		long userGroupId = userRepo.getById(userId).getUserGroupId();
